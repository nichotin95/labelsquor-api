...
async def health_check() -> HealthCheckResponse:
    ...
    return HealthCheckResponse(
        status="ok", timestamp=datetime.utcnow().isoformat(), version=settings.VERSION, database="unknown"
    )
...
async def readiness_probe(session: AsyncSessionDep) -> Dict[str, Any]:
    ...
    return {"status": "ok" if all_healthy else "degraded", "timestamp": datetime.utcnow().isoformat(), "checks": checks}
...
async def detailed_health(session: AsyncSessionDep) -> Dict[str, Any]:
    ...
    try:
        ...
        health_data["components"]["database"] = {
            "status": "healthy",
            "version": db_version,
            "metrics": {"products": product_count.scalar(), "brands": brand_count.scalar()},
        }
    except Exception as e:
    ...
    try:
        ...
        health_data["components"]["cache"] = {"status": "healthy", "type": cache.__class__.__name__}
    except Exception as e:
