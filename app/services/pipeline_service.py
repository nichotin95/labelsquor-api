...
class PipelineService:
    ...
    STAGE_ORDER = ["discovery", "image_fetch", "ocr", "enrichment", "scoring", "indexing"]
    ...
    def _get_scoring_service(self):
        ...
        return ScoringService(self.product_repo, FactsRepository())
    ...
    async def process_queue_item(self, queue_id: UUID) -> ProcessingQueue:
        ...
        await self.queue_repo.update_status(
            queue_id=queue_id, status="processing", processing_started_at=datetime.utcnow()
        )
        ...
        try:
            ...
            # Mark as completed
            await self.queue_repo.update_status(queue_id=queue_id, status="completed", completed_at=datetime.utcnow())
            ...
            if queue_item.retry_count < queue_item.max_retries:
                ...
                # Max retries reached
                await self.queue_repo.update_status(queue_id=queue_id, status="failed", last_error=str(e))
    ...
    async def _process_discovery(self, queue_item: ProcessingQueue):
        ...
        product = await self.product_repo.find_or_create_from_source(
            brand_name=data.get("brand", "Unknown"), product_name=data.get("name", "Unknown"), source_data=data
        )

        # Link to queue item
        await self.queue_repo.update(id=queue_item.queue_id, obj_in={"product_id": product.product_id})

        # Create new product version for this update
        version = await self.product_repo.create_version(product_id=product.product_id)
    ...
    async def _process_image_fetch(self, queue_item: ProcessingQueue):
        ...
        await self.queue_repo.update_stage_details(
            queue_id=queue_item.queue_id, stage="image_fetch", details={"images": stored_images}
        )
    ...
    async def _process_ocr(self, queue_item: ProcessingQueue):
        ...
        for image in images:
            if image["role"] in ["front", "back", "ingredients", "nutrition"]:
                try:
                    ...
                    ocr_results.append(
                        {"image": image["object_key"], "role": image["role"], "text": ocr_text, "confidence": 0.95}
                    )
        ...
        # Save OCR results
        await self.queue_repo.update_stage_details(
            queue_id=queue_item.queue_id, stage="ocr", details={"ocr_results": ocr_results}
        )
    ...
    async def _process_enrichment(self, queue_item: ProcessingQueue):
        ...
        if all_text:
            ...
            ingredients = await parsing_service.parse_ingredients(all_text)
            if ingredients:
                await self.product_repo.save_ingredients(
                    product_version_id=latest_version.product_version_id, ingredients=ingredients
                )

        # Parse nutrition
        nutrition_text = next((r["text"] for r in ocr_results if r["role"] == "nutrition"), None)
        if nutrition_text:
            parsing_service = self._get_parsing_service()
            nutrition = await parsing_service.parse_nutrition(nutrition_text)
            if nutrition:
                await self.product_repo.save_nutrition(
                    product_version_id=latest_version.product_version_id, nutrition=nutrition
                )

        # Extract allergens
        parsing_service = self._get_parsing_service()
        allergens = await parsing_service.extract_allergens(all_text)
        if allergens:
            await self.product_repo.save_allergens(
                product_version_id=latest_version.product_version_id, allergens=allergens
            )
    ...
    async def _process_scoring(self, queue_item: ProcessingQueue):
        ...
        scores = await scoring_service.calculate_squor(product_version_id=latest_version.product_version_id)

        # Save scores
        await self.product_repo.save_squor_scores(product_version_id=latest_version.product_version_id, scores=scores)
    ...
    async def _process_indexing(self, queue_item: ProcessingQueue):
        ...
        product_data = await self.product_repo.get_product_for_indexing(product_id=queue_item.product_id)
