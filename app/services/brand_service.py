...
class BrandService:
    ...
    async def create_brand(self, brand_data: BrandCreate) -> BrandRead:
        ...
        existing = await self.brand_repo.get_by_normalized_name(
            normalized_name, 
            brand_data.country
        )
        ...
        if existing:
            raise ConflictError(
                f"Brand '{brand_data.name}' already exists",
                existing_id=str(existing.brand_id)
            )
    ...
    async def update_brand(
        ...
        if brand_update.name and brand_update.name != brand.name:
            ...
            if existing and existing.brand_id != brand_id:
                raise ConflictError(
                    f"Brand name '{brand_update.name}' already exists"
                )
        ...
        # Update brand
        updated_brand = await self.brand_repo.update(
            id=brand_id,
            obj_in=brand_update
        )
    ...
    async def delete_brand(self, brand_id: UUID) -> bool:
        ...
        if brand_with_count["product_count"] > 0:
            raise BusinessLogicError(
                ...
                product_count=brand_with_count["product_count"]
            )
