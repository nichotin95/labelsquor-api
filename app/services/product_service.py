...
class ProductService:
    ...
    def __init__(self, product_repo: ProductRepository, brand_repo: BrandRepository, category_repo: CategoryRepository):
    ...
    async def create_product(self, product_data: ProductCreate) -> ProductRead:
        ...
        if product_data.gtin_primary:
            ...
            if existing:
                raise BusinessLogicError(
                    f"Product with GTIN {gtin} already exists", existing_product_id=str(existing.product_id)
                )
        ...
        product = await self.product_repo.create(
            obj_in=product_data, normalized_name=normalized_name, canonical_key=canonical_key
        )
