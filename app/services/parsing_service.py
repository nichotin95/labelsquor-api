...
class ParsingService:
    ...
    async def parse_ingredients(self, text: str) -> Optional[Dict[str, Any]]:
        ...
        ingredients_text = re.sub(r'(for more info|storage|store|keep|refrigerate).*$', '', ingredients_text, flags=re.IGNORECASE)
    ...
    async def extract_certifications(self, text: str) -> Optional[List[Dict[str, Any]]]:
        ...
        if fssai_match:
            certifications.append({
                "scheme": "FSSAI",
                "id_code": re.sub(r'\s+', '', fssai_match.group(1)),
                "confidence": 0.9
            })
